// ==UserScript==
// @name          比较语言学工具 Comparative Linguistic Tools
// @version      0.3
// @description  Extract text from <td class="ant-table-cell"> elements, save them to a text file, and notify user on success. 读取cell元素的内容导出到txt文本，并对使用者给出通知。
// @author       ajamaxan
            ...
            mutations.forEach(mutation => {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1 && node.matches('td.ant-table-cell')) {
                        extractAndSaveTextContent();
                        showNotification('文本内容已更新！'); // 显示通知
                    }
                });

                // 如果现有元素文本内容发生变化，重新提取数据并显示通知
